...
const destination: StudioPage = { id: 'bergen', order: 10, type: 'destination', role: 'destination', title: 'Bergen', content: 'content/pages/010-bergen.md', layout: 'destination-standard', components: ['hero', 'title', 'introduction', 'history', 'photography', 'knowledge', 'qr'] };
...
describe('editorial grammar library', () => {
  ...
  it('evaluates editorial completeness from required story components only', () => {
    ...
    expect(evaluation?.optionalAvailable.map((rule) => rule.id)).toContain('souvenirs');
  });
  ...
  it('reports missing required components in editorial language', () => {
    ...
    expect(evaluation?.completeness).toBe(83);
